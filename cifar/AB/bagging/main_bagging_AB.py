...
parser.add_argument('--data', action='store', default='./data/',
	help='dataset path')
parser.add_argument('--arch', action='store', default='allbinnet',
	help='the architecture for the network: nin')
parser.add_argument('--lr', action='store', default='0.0001',
	help='the intial learning rate')
parser.add_argument('--epochs', action='store', default='0',
	help='first train epochs',type=int)
parser.add_argument('--retrain_epochs', action='store', default='100',
	help='re-train epochs',type=int)
parser.add_argument('--save_name', action='store', default='allbin_model',
	help='save the first trained model',type=str)
parser.add_argument('--load_name', action='store', default='allbin_model',
	help='load pretrained model',type=str)
parser.add_argument('--root_dir', action='store', default='models_bagging_AB/',
...
def adjust_learning_rate(optimizer, epoch):
	...
	if epoch in update_list:
		for param_group in optimizer.param_groups:
			...
			return
def reset_learning_rate(optimizer, lr=0.0001):
	for param_group in optimizer.param_groups:
		param_group['lr'] = lr
	return
...
def sample_models(bagging_iters, sample_weights, retrain_epoch=100):
	print(str(datetime.datetime.utcnow())+" Start bagging iter: "+str(bagging_iters) )
	print('===> Start retraining ...')

	best_acc = 0
	reset_learning_rate(optimizer, lr=0.0001)
	for epoch in range(1, retrain_epoch+1):
		this_train_loader = train(epoch, sample_weights)
		if epoch % 5 ==0:
			best_acc = test(str(bagging_iters), best_acc, this_train_loader)
			eval_test()
...
if __name__ == '__main__':
	...
	for i in range(bagging_iters):
		print("Bagging "+str(i))
		sample_weights_new = np.random.choice(50000, size=50000)
		sample_models(bagging_iters=i, sample_weights=sample_weights_new, retrain_epoch=args.retrain_epochs)
	...
	for num_bagging in range(1,32):
		...
		for data, target, _ in testloader:
			...
			for i in range(num_bagging):
				...
				pred_test = combine_softmax_output(pred_test_i.data.cpu(), pred_test, np.ones((1,bagging_iters)), i)
